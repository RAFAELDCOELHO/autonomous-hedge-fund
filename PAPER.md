...
This is a research-in-progress report: infrastructure, agent, and validation are complete; the factorial experiment has not yet been executed. The design is documented to be auditable before results exist, and result-dependent sections are explicitly marked as pending. Section 6A records committed offline BrazilBench artifacts (P1.5–P1.10) that are outside `make reproduce`; H1 remains pending.
...
## 6A. BrazilBench offline artifacts

The artifacts below are committed under `benchmark/results/` and are regenerated by dedicated Makefile targets. They are **outside** `make reproduce`. Each subsection states only what the corresponding artifact supports. None of them tests H1, which remains **[PENDING]**.

### 6A.1 Qwen cold-start (PR #8)

`make qwen-n10` writes `benchmark/results/qwen_n10/`. The committed run is qwen2.5:7b at temperature 0, N=10 independent cold-start sessions on three PETR4 / crisis_2020 dates. Per-date agreement is 1.0, confidence is 0.7 ± 0.0, and each date has 1 unique response. The artifact supports this local cold-start protocol; it does not support alpha, calibration, or generalization. The target needs a local Ollama install with `qwen2.5:7b` and is not part of the $0 reproduce path.

### 6A.2 Reliability scaffold (PR #7)

`make reliability` writes `benchmark/results/reliability/`. The committed `bins.csv` has a single bin 0.9–1.0 with n=6, n_unique_prompts=1, mean_conf 0.9, and win_rate 0.0 (mistral:7b, PETR4, 2020-03-06, BUY followed by a next-day loss). This is an offline scaffold, not a Claude calibration result. The same caveats are already stated in Section 8.

### 6A.3 HMM label alignment (PR #9)

`make hmm-regimes` writes `benchmark/results/hmm_regimes/`. Day-weighted purity over the four hand windows is 0.949; the adjusted Rand index is 0.040; crisis_2020 purity is 0.55 (45% of days labelled turbulent). The artifact is label agreement only: it does not re-run strategies under HMM windows and involves no LLM.

### 6A.4 Multi-asset portfolios (PR #11)

`make multi-asset` writes `benchmark/results/multi_asset/`. Mean off-diagonal correlation of in-regime daily returns in crisis_2020 is 0.814. Inverse-vol and min-variance weights use a 252-day warmup strictly before each regime (no look-ahead). Example: Buy & Hold, bull_2019, equal-weight daily portfolio 19.12% versus the appendix single-ticker equal-weight mean 17.23%. No LLM is run in a multi-asset setting; execution is frictionless.

### 6A.5 Survivorship bracket (PR #10)

`make survivorship` writes `benchmark/results/survivorship/`. Distressed names are OIBR3, MGLU3, and AMER3 (GOLL4 is unavailable on Yahoo). Overall mean liquid-minus-distressed gap is +6.66 pp; the hi_rates_2022 mean gap across agents is +42.98 pp; Buy & Hold in hi_rates_2022 is +82.83 pp. Three names only; no significance test.

### 6A.6 Chronos-t5-tiny (PR #12)

`make chronos` writes `benchmark/results/chronos/`. PETR4 mean CR is 1.68% (gap vs Buy & Hold −8.80 pp); ^BVSP mean CR is −1.87% (gap −5.99 pp). This is a tiny-checkpoint, frictionless forecast-sign comparator, not a foundation-model alpha claim.

### 6A.7 Scope relative to H1

These artifacts do not resolve H1. The $0 reproduce path remains only `make reproduce`.
...
**Confidence calibration [PENDING — Claude runs; $0 scaffold shipped].** The Signal Processor emits a confidence score alongside each discrete trading signal, but whether those confidences are *calibrated* — i.e., whether higher-confidence decisions are more often correct on the following day — is untested for Claude. The analysis pipeline exists and runs offline (`make reliability`): it normalises existing local decision logs into `benchmark/results/reliability/decisions.jsonl`, joins each decision with the next trading day's Close from the committed PETR4 fixture, bins by stated confidence and writes bin mean-confidence against realised win rate (`bins.csv`, `reliability.svg`). The only local logs with a confidence field are six temperature-0 replicates of a single mistral:7b prompt (PETR4, 2020-03-06, BUY at 0.9, followed by a next-day loss); the 480 rows of the 80-day mistral:7b run carry signals but no confidence and are reported only as a base rate. One unique decision is not a calibration estimate and we draw no conclusion from it. A Claude reliability diagram over the ticker/regime grid, across multiple seeds, requires paid API runs that log per-decision confidence in this schema; it has not been executed and no calibration result is fabricated in the interim. (The zero-cost classical baselines — a RandomAgent null distribution over $N=100$ seeds and equal-weight rule-based portfolios across four regime windows — are reported separately as `docs/paper_random_n100.tex` and `docs/paper_ew_portfolio_baselines.tex`.)
