...
**Single-run LLM stochasticity and statistical criteria.** Multi-agent LLM pipelines are stochastic; robust conclusions will require multiple runs per cell and seeds/temperature reporting, which the current plan does not yet fix. Relatedly, H1's "significantly" is not yet backed by a signed-off statistical criterion. A draft pre-registration awaiting author sign-off (`docs/PREREGISTRATION.md`, implemented in `scripts/h1_stats.py`) specifies an exact one-sided permutation test on per-ticker Sharpe deltas (Brazil macro-sensitive vs. US) at α = 0.05, market-specific risk-free rates (CDI for Brazil, the 3-month T-bill for the US), exclusion rules, and a Holm-corrected secondary family.
